...
def create_split(cfg: SplitConfig, force_rebuild: bool = False) -> dict:
    df = load_hourly_data()
    assignments = ensure_split_assignments(
        df=df,
        cfg=cfg,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create persisted train/val/test split assignments.")
    parser.add_argument(
        "--strategy",
...
if __name__ == "__main__":
    ...
    stats = create_split(cfg=config, force_rebuild=args.force_rebuild)
