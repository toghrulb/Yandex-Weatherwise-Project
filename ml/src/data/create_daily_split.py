...
from ml.src.data.daily_split_manager import ensure_daily_split_assignments
from ml.src.data.load_daily_data import load_daily_data
from ml.src.data.split_manager import SplitConfig


def create_daily_split(cfg: SplitConfig, force_rebuild: bool = False) -> dict:
    df = load_daily_data()
    assignments = ensure_daily_split_assignments(
        df=df,
        cfg=cfg,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create persisted train/val/test split for daily dataset.")
    parser.add_argument(
        "--strategy",
...
if __name__ == "__main__":
    ...
    stats = create_daily_split(cfg=config, force_rebuild=args.force_rebuild)
    print("Daily split creation complete.")
