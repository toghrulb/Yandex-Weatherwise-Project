...
from ml.src.utils.paths import DAILY_UMBRELLA_MODEL_PATH, DAILY_UMBRELLA_TRAINING_REPORT_PATH


@dataclass
class DailyUmbrellaCandidate:
    name: str
    c: float
    class_weight: str | None


def _candidate_grid() -> list[DailyUmbrellaCandidate]:
    return [
        DailyUmbrellaCandidate(name="logreg_c0.5_balanced", c=0.5, class_weight="balanced"),
        DailyUmbrellaCandidate(name="logreg_c1.0_balanced", c=1.0, class_weight="balanced"),
        DailyUmbrellaCandidate(name="logreg_c2.0_balanced", c=2.0, class_weight="balanced"),
        DailyUmbrellaCandidate(name="logreg_c1.0_unweighted", c=1.0, class_weight=None),
    ]


def train_daily_umbrella_model(
    force_rebuild_split: bool = False,
    split_config: SplitConfig | None = None,
) -> dict:
    frames = load_daily_target_frames(
        target_column="umbrella_recommended",
    ...
    save_daily_model(DAILY_UMBRELLA_MODEL_PATH, best["model"])
    report = {
        "target": "umbrella_recommended",
        "model_path": str(DAILY_UMBRELLA_MODEL_PATH),
    ...
    save_daily_training_report(DAILY_UMBRELLA_TRAINING_REPORT_PATH, report)
    return report


if __name__ == "__main__":
    out = train_daily_umbrella_model(force_rebuild_split=False)
    print("Daily umbrella training complete.")
