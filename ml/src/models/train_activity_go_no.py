...
from ml.src.utils.paths import ACTIVITY_GO_NO_MODEL_PATH, ACTIVITY_GO_NO_TRAINING_REPORT_PATH


@dataclass
class ActivityGoNoCandidate:
    name: str
    c: float
    class_weight: str | None


def _candidate_grid() -> list[ActivityGoNoCandidate]:
    return [
        ActivityGoNoCandidate(name="logreg_c0.5_balanced", c=0.5, class_weight="balanced"),
        ActivityGoNoCandidate(name="logreg_c1.0_balanced", c=1.0, class_weight="balanced"),
        ActivityGoNoCandidate(name="logreg_c2.0_balanced", c=2.0, class_weight="balanced"),
        ActivityGoNoCandidate(name="logreg_c1.0_unweighted", c=1.0, class_weight=None),
    ]


def train_activity_go_no_model(
    ...
    save_activity_model(ACTIVITY_GO_NO_MODEL_PATH, best["model"])
    report = {
        "target": "go_or_no",
        "model_path": str(ACTIVITY_GO_NO_MODEL_PATH),
    ...
    save_activity_training_report(ACTIVITY_GO_NO_TRAINING_REPORT_PATH, report)
    return report


if __name__ == "__main__":
    out = train_activity_go_no_model(force_rebuild_split=False)
    print("Activity go/no-go training complete.")
